...
import { photometricInterpretations, parseXml } from './globals.js';
...
export default class GeoTIFFImage {
  constructor(fileDirectory, geoKeys, dataView, littleEndian, cache) {
    ...
    let planarConfiguration = fileDirectory.PlanarConfiguration;
  ...
  getBytesPerPixel() {
    ...
    for (let i = 0; i < this.fileDirectory.BitsPerSample.length; ++i) {
      let bits = this.fileDirectory.BitsPerSample[i];
      if ((bits % 8) !== 0) {
  ...
  getReaderForSample(sampleIndex) {
    let format = this.fileDirectory.SampleFormat ? this.fileDirectory.SampleFormat[sampleIndex] : 1;
    let bitsPerSample = this.fileDirectory.BitsPerSample[sampleIndex];
    switch (format) {
      case 1: // unsigned integer data
        switch (bitsPerSample) {
          case 8:
  ...
  getTileOrStrip(x, y, sample, pool) {
    let numTilesPerRow = Math.ceil(this.getWidth() / this.getTileWidth());
    let numTilesPerCol = Math.ceil(this.getHeight() / this.getTileHeight());
    let index;
    ...
    let slice = this.dataView.buffer.slice(offset, offset + byteCount);
    ...
    for (let yTile = minYTile; yTile < maxYTile; ++yTile) {
      for (let xTile = minXTile; xTile < maxXTile; ++xTile) {
        for (let sampleIndex = 0; sampleIndex < samples.length; ++sampleIndex) {
          const _sampleIndex = sampleIndex;
          const sample = samples[_sampleIndex];
          ...
          promise.then((tile) => {
            ...
            const reader = sampleReaders[_sampleIndex];
            ...
            for (let y = Math.max(0, imageWindow[1] - firstLine); y < ymax; ++y) {
              for (let x = Math.max(0, imageWindow[0] - firstCol); x < xmax; ++x) {
                ...
                const value = reader.call(dataView, pixelOffset + srcSampleOffsets[_sampleIndex], littleEndian);
