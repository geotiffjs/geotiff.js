...
const geotiff = require('../dist/bundle-node.js');

function createSource(filename) {
  return geotiff.fromFile(`test/data/${filename}`);
}
...
describe('GeoTIFF', () => {
  ...
  it('should work on tiled tiffs', async () => {
    const tiff = await createSource('tiled.tiff');
    await performTiffTests(tiff, 539, 448, 15, Uint16Array);
  });

  it('should work on band interleaved tiffs', async () => {
    const tiff = await createSource('interleave.tiff');
    await performTiffTests(tiff, 539, 448, 15, Uint16Array);
  });
...
describe('RGB-tests', () => {
  ...
  const comparisonRaster = (async () => {
    const tiff = await createSource('rgb.tiff');
    const image = await tiff.getImage();
